...
const exportedMethods = {
  async createCompanies (
    ...
    if (!insertInfo.acknowledged || !insertInfo.insertedId)
      throw 'Error: Could not add new company'
  ...
  async removeCompany (id) {
    ...
    const companyToRemove = await companiesCollection.findOneAndDelete({
      _id: companyId
    })
  ...
  async updateCompany (id, name, description, size, location) {
    ...
    if (size) updateData.size = xss(size)
    ...
    if (!updatedInfo)
      throw new Error(`Update failed, could not find company with id of ${id}`)
  },
  ...
  async getCompaniesByPage (page, limit) {
    ...
    const companiesCollection = await companies()
    const companiesList = await companiesCollection
      .find({})
      .skip(skip)
      .limit(limit)
      .toArray()
