...
router
  ...
  .post(async (req, res) => {
    ...
    if(!companyInfo || Object.keys(companyInfo).length === 0) {
      return res.status(400).json({error: 'You must provide data to create a company'});
    }
    try {
...
router
  ...
  .put(async (req, res) => {
    ...
    try {
      ...
      requestBody.name = validations.checkString(requestBody.name, "Company's name");
      requestBody.description = validations.checkString(requestBody.description, "Company's description");
  ...
  .patch(async (req, res) => {
    ...
    if (!companyInfo || Object.keys(companyInfo).length === 0) {
      return res.status(400).json({error: 'You must provdie data to update a company'});
    }
    try {
      companyId = validations.checkId(companyId, "Company ID");

      if (companyInfo.name) {
        companyInfo.name = validations.checkString(companyInfo.name, "Company's Name");
      }
      if (companyInfo.description) {
        companyInfo.name = validations.checkString(companyInfo.description, 'Description');
      }
      if(companyInfo.size) {
        companyInfo.size = validations.checkString(companyInfo.size, 'Size');
      }
      if (companyInfo.location) {
        companyInfo.location = validations.checkString(companyInfo.location, 'Location');
      }
    } catch (e) {
      return res.status(400).json({ error: e.message })
    }
...
router
 ...
 .post(async (req, res) => {
    try {
      ...
      const companies = await companiesData.getCompanyByTitle(searchByTitle.trim());

      if (companies.length === 0) {
        res.status(404).render('error', {error: `We're sorry, but no results were found for ${searchByTitle}`});
      } else {
